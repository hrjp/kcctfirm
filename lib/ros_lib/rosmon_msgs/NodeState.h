  ...
  class NodeState : public ros::Msg
  {
    ...
    virtual int serialize(unsigned char *outbuffer) const
    {
      ...
      *(outbuffer + offset + 3) = (u_system_load.base >> (8 * 3)) & 0xFF;
      ...
      u_memory.real = this->memory;
      *(outbuffer + offset + 0) = (u_memory.base >> (8 * 0)) & 0xFF;
      *(outbuffer + offset + 1) = (u_memory.base >> (8 * 1)) & 0xFF;
      *(outbuffer + offset + 2) = (u_memory.base >> (8 * 2)) & 0xFF;
      *(outbuffer + offset + 3) = (u_memory.base >> (8 * 3)) & 0xFF;
    ...
    virtual int deserialize(unsigned char *inbuffer)
    {
      ...
      u_memory.base = 0;
      u_memory.base |= ((uint32_t) (*(inbuffer + offset + 0))) << (8 * 0);
      u_memory.base |= ((uint32_t) (*(inbuffer + offset + 1))) << (8 * 1);
      u_memory.base |= ((uint32_t) (*(inbuffer + offset + 2))) << (8 * 2);
      u_memory.base |= ((uint32_t) (*(inbuffer + offset + 3))) << (8 * 3);
      this->memory = u_memory.real;
      offset += sizeof(this->memory);
     return offset;
    }

    const char * getType(){ return "rosmon_msgs/NodeState"; };
    const char * getMD5(){ return "87292cdbe5a500a95e32714adf10e6dd"; };
