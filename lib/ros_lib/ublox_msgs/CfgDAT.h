...
namespace ublox_msgs
{
  ...
  class CfgDAT : public ros::Msg
  {
    public:
      ...
      uint8_t datumName[6];
      typedef float _majA_type;
      _majA_type majA;
      typedef float _flat_type;
      _flat_type flat;
    ...
    virtual int serialize(unsigned char *outbuffer) const
    {
      ...
      *(outbuffer + offset + 0) = (this->datumName[i] >> (8 * 0)) & 0xFF;
      ...
      offset += serializeAvrFloat64(outbuffer + offset, this->majA);
      offset += serializeAvrFloat64(outbuffer + offset, this->flat);
      union {
        float real;
        uint32_t base;
      } u_dX;
    ...
    virtual int deserialize(unsigned char *inbuffer)
    {
      ...
      offset += deserializeAvrFloat64(inbuffer + offset, &(this->majA));
      offset += deserializeAvrFloat64(inbuffer + offset, &(this->flat));
      union {
        float real;
        uint32_t base;
      } u_dX;
