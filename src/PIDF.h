...
class PID{
    ...
    //ki:積分定数
    //kd:微分定数
    PID(float kp,float ki,float kd,float kf=0.0);
    ...
    //ki:積分定数
    //kd:微分定数
    void set(float kp,float ki,float kd,float kf=0.0);
...
PID::PID(float kp,float ki,float kd,float kf){
    kpp=kp;
    kii=ki;
    kdd=kd;
    kff=kf;
...
void PID::set(float kp,float ki,float kd,float kf){
    kpp=kp;
    kii=ki;
    kdd=kd;
    kff=kf;
...
float PID::update(float local_val,float target_val,unsigned long loop_cycle_ms){
    ...
    return result_value+kff*target_val;
}
